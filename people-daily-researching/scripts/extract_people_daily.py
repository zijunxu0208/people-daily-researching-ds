#!/usr/bin/env python3
"""
Extract People's Daily e-paper reports for a given date.
Produces:
...
def discover_nodes(layout_url):
    ...
    for href in re.findall(r'href=["\']?([^"\'>\s]*node_\d+\.html)["\']?', html):
        ...
        if name not in seen and "/rmrb/pc/layout/" in abs_href:
            seen.add(name)
...
def extract_article_links(html, base_url):
    ...
    for href in re.findall(r'href=["\']?([^"\'>\s]*content_\d+\.html)["\']?', html):
        ...
        if abs_href not in seen and "/rmrb/pc/content/" in abs_href:
            seen.add(abs_href)
...
def build_html(records, date_str):
    lines = [
        ...
        "<head>",
        '<meta charset="UTF-8">',
        f"<title>人民日报 {date_str} 报道摘要</title>",
        "<style>"
...
def main():
    ...
    json_path = os.path.join(out_dir, f"人民日报_{date_str}_报道摘要.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=2)

    html_path = os.path.join(out_dir, f"人民日报_{date_str}_报道摘要.html")
