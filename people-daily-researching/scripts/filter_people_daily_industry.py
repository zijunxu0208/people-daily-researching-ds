#!/usr/bin/env python3
"""
Filter People's Daily extraction by industry keywords.
Produces:
  - 人民日报_YYYYMMDD_行业筛选.html
  - 人民日报_YYYYMMDD_行业筛选.json (when matches exist)
"""
import argparse
import json
import os
...
def main():
    ...
    date_match = re.search(
        r"人民日报_(\d{8})_报道摘要\.json", os.path.basename(args.json_file)
    )
    ...
    if matched:
        json_path = os.path.join(args.out, f"人民日报_{date_str}_行业筛选.json")
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(matched, f, ensure_ascii=False, indent=2)

    html_path = os.path.join(args.out, f"人民日报_{date_str}_行业筛选.html")
    lines = [
        "<!DOCTYPE html>",
        '<html lang="zh-CN">',
        "<head>",
        '<meta charset="UTF-8">',
        f"<title>人民日报 {date_str} 行业筛选</title>",
        "<style>"
