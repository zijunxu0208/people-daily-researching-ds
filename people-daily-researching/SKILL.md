...
description: "Extract all page reports from the People's Daily / 人民日报 e-paper for a specific date, including 版面, article titles, short summaries, and PDF links. Use when the user asks to 提取/抓取/整理/汇总人民日报电子版, 人民日报某天所有版面报道, People's Daily e-paper, or provides a paper.people.com.cn/rmrb URL, YYYYMMDD date, or Chinese date for this purpose."
---

# People Daily E-paper Extraction

Extract People's Daily e-paper reports for one date and produce an HTML table with:
...
## Preferred Path: Inline Python

When a Python runtime is available, execute the following exact Python program with the requested date. Do not require shell if the environment can run Python code directly. If files are available, save the program as `extract_people_daily.py` and call its `main()`/CLI with the target date; otherwise adapt only the final output step to return the HTML/JSON content through the available artifact channel.

Set the date argument to the user-requested date, for example `20260630`. Set `--out` only when writing files is supported.
...
The program uses only Python standard library modules. It discovers all layout pages from `node_01.html`, extracts each page PDF link, visits article HTML pages, takes the first meaningful paragraph as a summary, and writes or returns:

- `人民日报_YYYYMMDD_报道摘要.html`
- `人民日报_YYYYMMDD_报道摘要.json`

If Python/network execution is restricted, use the Browser-Only Path below instead of inventing URL variants or relying on shell workarounds.

## Browser-Only Path

Use this path when the agent cannot read/write local files or cannot run Python, but can control a browser.

1. Open the first layout URL using the `YYYYMM/DD` pattern.
2. In the page DOM, collect anchors whose `href` matches `/rmrb/pc/layout/YYYYMM/DD/node_XX.html` and visible text contains `版`.
3. Visit each layout page.
4. On each layout page, collect:
   - page label from text like `第01版：要闻`
   - PDF from link text containing `PDF` or href ending `.pdf`
   - article anchors matching `/rmrb/pc/content/YYYYMM/DD/content_*.html`
5. Visit each article URL and extract the first meaningful paragraph from `<p>` elements. Skip navigation/noise paragraphs such as `日报`, `周报`, `杂志`, `本版新闻`, `返回目录`, `放大`, `缩小`, `全文复制`, `上一篇`, `下一篇`, `版权声明`, page labels, date lines, source lines like `《人民日报》（YYYY年MM月DD日 第 XX 版）`, and short photo-credit lines ending in `摄`.
6. Keep summaries to at most 200 Chinese characters.
7. If output files are not possible, return the table in chat or in the browser-supported artifact format.

Useful browser DOM snippets:

```js
// Layout page extraction. Run in the page context.
(() => {
  ...
  const pdf = links.find(l => /PDF/i.test(l.text) || /\.pdf(\?|$)/i.test(l.href))?.href || "";
...
```js
// Article summary extraction. Run in the article page context.
((expectedTitle) => {
...
After the main HTML and JSON are generated, ask the customer exactly:

`是否搜集“旅游”、“美妆”、“日化”行业相关消息？若不搜索上述消息，请给出您的筛选词：___`

Then filter the extracted article records using the customer's response:

- If the customer provides custom screening words, split them on whitespace, Chinese/English commas, semicolons, slashes, and newlines. Use those words as the filter criteria.
- If the customer does not answer with corresponding screening words, or answers only yes/confirm/blank, default to the built-in industry criteria below.
- Match against article title plus extracted summary/body text. When full body text is available from browser extraction, include it in the match text; otherwise use the title plus summary from the generated JSON.
- Preserve original page, title, summary, article URL, and PDF URL for each matched article.
- If files can be written, create an additional filtered deliverable named `人民日报_YYYYMMDD_行业筛选.html` and, when practical, `人民日报_YYYYMMDD_行业筛选.json`. If files cannot be written, return the filtered table in chat or the available artifact channel.
- If there are no matches, clearly report `未筛选到相关消息` and still state which screening words were used.
...
- When the main extraction finishes, report the extraction date, page count, article count, failure count, and the generated HTML path or delivered table location, then perform the Industry Follow-up Step.
